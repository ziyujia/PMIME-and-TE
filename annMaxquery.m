function [nnidx, dists] = annMaxquery(Xr, Xq, k, varargin)
% ANNMAXQUERY Performs Approximate K-Nearest-Neighbor query for a set of
% points using the maximum (infinity) norm. Otherwise it is the same as 
% 'annquery.m'
% 
% [ Syntax ]
...
% The following lines are the only modifications in the code of 'annquery.m' 
[nnidx, dists] = annMax_mex(Xr, Xq, internal_opts);

nnidx = nnidx + 1;        % from zero-based to one-based        
if nargout >= 2
    % The following line is commented because the maximum norm does not
    % give out squared distances
