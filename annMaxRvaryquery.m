...
% ANNQUERY Performs Approximate Nearest-Neighbor query within a fixed
% distance for a set of points using the maximum norm and allowing for a 
% different distance for each query point.
% 
% [ Syntax ]
%   - nnnidx = annMaxRvaryquery(Xr, Xq, R, k, 'search_sch', 'fr', 'radius', a);
%
% INPUTS
% - Xr:         the reference points to construct the kd-tree (d x n matrix)
% - Xq:         the query points (d x nq matrix)
% - R:          the distances for the query points (nq x 1 matrix)
% - k:          dummy variable, use any real scalar, no effect
% - a           dummy variable, use any real scalar, no effect
% OUTPUTS
% - nnnidx:     the array of number of nearest neighbors for each query
%               point (1 x nq matrix)
%
% The text below after the title [Description] is copied from function
% 'annquery' for general search.  
% The differences of 'annMaxRvaryquery' from 'annquery' are the following: 
% 1. 'annMaxRvaryquery' is restricted to work only for the option of 'fixed
% radius search'. 
% 2. 'annMaxRvaryquery' uses maximum norm (infinity norm) for the distance
% calculations.
% 3. 'annMaxRvaryquery' gets as input an array of distances for the 'fixed
% radius search', one for each query point. 
% [ Description ]
...
%   - Modified by Dimitris Kugiumtzis, 30 Dec 2010.
...
% The following line is the only modification in the code of 'annquery.m' 
